...
public class CreateBrandTests : BrandMockRepository
{
    ...
    public CreateBrandTests(BrandFakeData fakeData, CreateBrandCommandValidator validator, CreateBrandCommand command)
        : base(fakeData)
    {
        _command = command;
    ...
    {
        _command.Name = "Audi";
        CreatedBrandResponse result = await _handler.Handle(_command, CancellationToken.None);
        Assert.Equal(expected: "Audi", result.Name);
    }
