...
namespace Core.Infrastructure.ElasticSearch;
...
public class ElasticSearchManager : IElasticSearch
{
        ...
        where T : class
    {
        ...
        ISearchResponse<T>? searchResponse = await elasticClient.SearchAsync<T>(
                                                 s => s.Index(Indices.Index(parameters.IndexName)).From(parameters.From)
                                                       .Size(parameters.Size)
                                             );

        List<ElasticSearchGetModel<T>> list = searchResponse.Hits
                                                            .Select(x => new ElasticSearchGetModel<T>
        ...
        where T : class
    {
        ...
        ISearchResponse<T>? searchResponse = await elasticClient.SearchAsync<T>(
                                                 s => s.Index(fieldParameters.IndexName).From(fieldParameters.From)
