...
namespace Modules.BaseApplication.Features.Brands.Rules;
...
public class BrandBusinessRules : BaseBusinessRules
{
    ...
    {
        Brand? result =
            await _brandRepository.GetAsync(x => x.Id != brand.Id && x.Name.ToLower() == brand.Name.ToLower());
        if (result != null)
            throw new BusinessException(BrandsMessages.BrandNameExists);
    }
    ...
    {
        IPaginate<Brand> result =
            await _brandRepository.GetListAsync(predicate: b => nameList.Contains(b.Name), enableTracking: false);
        if (result.Items.Any())
            throw new BusinessException(BrandsMessages.BrandNameExists);
    }
}